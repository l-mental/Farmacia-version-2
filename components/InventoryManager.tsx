      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 gap-4 md:gap-6">
          {filtered.length === 0 ? (
            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-12 md:p-20 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg md:text-xl font-black text-slate-800 mb-2">No se encontraron resultados</h3>
              <p className="text-slate-400 max-w-xs text-sm">Intenta ajustar tu búsqueda o agrega un nuevo producto al inventario.</p>
            </div>
          ) : (
            filtered.map(med => (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-slate-900/60 backdrop-blur-md p-0 md:p-4 animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-4xl md:rounded-[2.5rem] lg:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-[92vh] md:h-auto md:max-h-[90vh] animate-in slide-in-from-bottom-10 duration-500">
              <div className="p-5 md:p-8 lg:p-10 bg-slate-900 text-white flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="p-2.5 md:p-3 bg-emerald-500 rounded-xl md:rounded-2xl">
                    <Pill className="w-5 h-5 md:w-6 md:h-6 text-white" />
                  </div>
                  <div>
                    <h2 className="text-lg md:text-xl lg:text-2xl font-black">{editingMed ? 'Editar Producto' : 'Nuevo Medicamento'}</h2>
                    <p className="text-emerald-400 text-[8px] md:text-[10px] font-black uppercase tracking-widest">Información técnica y existencias</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 md:p-8 lg:p-10 space-y-8 md:space-y-10 no-scrollbar">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <select 
                        ...
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl md:rounded-2xl outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all text-sm font-bold text-slate-700 appearance-none cursor-pointer"
                      >
                ...
                <div className="space-y-4 md:space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-center bg-slate-50 p-4 rounded-2xl md:rounded-3xl border border-slate-100">
                        <div className="w-20 h-20 md:w-24 md:h-24 bg-white border border-slate-200 rounded-xl md:rounded-2xl flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                          {formData.imageUrl ? (
                            <img src={formData.imageUrl} alt="Preview" className="w-full h-full object-cover" />
                          ) : (
                            <ImageIcon className="text-slate-200 w-8 h-8 md:w-10 md:h-10" />
                          )}
                        </div>
                        <div className="flex-1 w-full space-y-3">
                          ...
                          <div className="relative">
                            ...
                            <label 
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest cursor-pointer hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20 w-full justify-center sm:w-auto"
                            >
                ...
                <div className="space-y-4 md:space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 bg-slate-50 p-5 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100">
                    <InputGroup 
                      label="Precio x Caja" 
                      type="number" 
                      step="0.01"
                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-4 pt-6 border-t border-slate-100">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="w-full md:flex-1 py-4 md:py-5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-black rounded-2xl md:rounded-[2rem] text-xs md:text-sm uppercase tracking-widest transition-all">Cancelar</button>
                  <button type="submit" className="w-full md:flex-[2] py-4 md:py-5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl md:rounded-[2rem] shadow-2xl shadow-emerald-600/30 text-xs md:text-sm uppercase tracking-widest transition-all active:scale-95">
